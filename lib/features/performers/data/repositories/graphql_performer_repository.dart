...
class GraphQLPerformerRepository implements PerformerRepository {
  ...
  @override
  Future<List<Performer>> findPerformers({int? page, int? perPage, String? filter}) async {
    ...
    final List performersJson = result.data?['findPerformers']?['performers'] ?? [];
    
    return performersJson.map((p) => Performer(
      id: p['id'],
      name: p['name'] ?? '',
      details: p['details'],
      gender: null,
